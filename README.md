...
- 生成岗位匹配分析报告，包括匹配等级、候选人优势、风险点、简历优化建议和面试关注点。
...
7. `POST /api/job/match` 根据 `resume_id` 和 JD 文本计算匹配评分，并生成可解释分析报告。
...
匹配接口还会基于评分明细生成 `analysis` 报告：

- `match_level`：根据总分划分为高匹配、中匹配或低匹配。
- `strengths`：提炼已命中的岗位关键词、经验、学历和意向优势。
- `risks`：指出缺失关键词、经验不足、学历不满足或求职意向缺失等风险。
- `suggestions`：给出可执行的简历优化方向。
- `interview_focus`：生成面试时建议重点追问的问题。
